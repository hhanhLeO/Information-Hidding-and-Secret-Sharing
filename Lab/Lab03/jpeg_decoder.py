...
def idct_naive(coeff):
    ...
    for i in range(64):
        ...
        for j in range(64):
            ...
            s += cu * cv * coeff[j] * \
                cos16((2 * x + 1) * u) * cos16((2 * y + 1) * v)
        block[i] = s / 4
    return block


# inverse DCT (LUT implementation)
COS_LUT = [[math.cos((2 * xy + 1) * uv * math.pi / 16)
            for uv in range(8)] for xy in range(8)]
...
def decode_block8x8(r, hdec, pred, qtbl, bpp):
    ...
    # *** Get block's quantized DCT coefficients
    blk_quant_dct_coefs = np.array(sq, dtype=np.int32)[ZZ]
    ...
    return (block, pred, blk_quant_dct_coefs)
...
def decode_scan(r, image, scan):
    ...
    # decode MCUs
    quant_dct_coefs = np.array([], dtype=np.int32)  # ***
    ...
    for mcu_idx in range(nmcu[0] * nmcu[1]):
        ...
        for ci, bx, by in blocks:
            ...
            blkimg, pred[ci], blk_quant_dct_coefs = decode_block8x8(
                r, hdec[ci * 2:ci * 2 + 2], pred[ci], qtbl[ci], bpp)
            quant_dct_coefs = np.append(
                quant_dct_coefs, blk_quant_dct_coefs)  # ***
    ...
    image['quant_dct_coefs'] = quant_dct_coefs  # ***
...
def parse_DHT(r, image):
    ...
    while 0 < lh:
        ...
        hc_s = ', '.join(['{n:0{s}b}'.format(n=c, s=hs[i])
...
def get_quant_dct_coefs_and_quant_table(infile):
    with Reader(infile) as r:
        image = parse_stream(r)
        quant_table = np.array(image['QT'][0])
        quant_table = quant_table[ZZ].reshape(8, 8)
        return image['quant_dct_coefs'], quant_table
